...
export const InstagramPreviewContent = ({
  ...
  const renderMediaContent = () => {
    if (showSlideshow && slideshowUrl) {
      return <div className="aspect-square bg-black rounded-lg overflow-hidden mb-3">
          <video src={slideshowUrl} className="w-full h-full object-cover" controls autoPlay muted loop />
        </div>;
    } else if (selectedImages.length > 0) {
      return <div className="aspect-square bg-muted rounded-lg overflow-hidden mb-3">
          <img src={selectedImages[0]} alt="Preview" className="w-full h-full object-contain" />
        </div>;
    }
    return null;
  };
  return <div className="glass border border-border/40 rounded-lg p-4">
      ...
      {isLoading ? <div className="flex items-center justify-center py-8">
          ...
          <div className="flex flex-col space-y-4">
            ...
            {!showSlideshow && images.length > 0 && <>
                ...
                <ScrollArea className="h-[400px] pr-4">
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 mb-4">
                    {images.map((image, index) => <div key={index} className="relative group border-2 border-transparent hover:border-pink-500 transition-all duration-200 rounded-lg">
                        ...
                        <div className="absolute top-2 left-2 bg-black/50 p-1.5 rounded">
                          <Checkbox checked={selectedImages.includes(image)} onCheckedChange={() => handleImageSelect(image)} disabled={!selectedImages.includes(image) && selectedImages.length >= 10} className="data-[state=checked]:bg-pink-500 data-[state=checked]:border-pink-500" />
                        </div>
            ...
            <Textarea value={generatedText} onChange={e => onTextChange(e.target.value)} className="min-h-[150px]" placeholder="Entrez votre texte ici..." />
