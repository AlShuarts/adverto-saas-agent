...
export const SoldBannerStatus = ({
  listing
}: SoldBannerStatusProps) => {
  ...
  const fetchRenders = async () => {
    try {
      ...
      const {
        data,
        error
      } = await supabase.from("sold_banner_renders").select("*").eq("listing_id", listing.id).order("created_at", {
        ascending: false
      });
      if (error) {
  ...
  const checkRenderStatus = async (renderId: string) => {
    try {
      ...
      const {
        data,
        error
      } = await supabase.functions.invoke('check-render-status', {
        body: {
          renderId
        }
      });
  ...
  useEffect(() => {
    const channel = supabase.channel('sold_banner_renders_changes').on('postgres_changes', {
      event: '*',
      schema: 'public',
      ...
      fetchRenders();
      if (payload.eventType === 'UPDATE' && payload.new.status === 'completed' && payload.old.status === 'pending') {
  ...
  if (latestRender.status === "pending") {
    return <div className="flex flex-col items-center">
        ...
        {hasError && <Alert variant="destructive" className="mt-4 mb-2">
            <AlertTriangle className="h-4 w-4" />
            <AlertDescription>{errorMessage || "Une erreur s'est produite."}</AlertDescription>
          </Alert>}
        <Button variant="outline" size="sm" className="mt-3" onClick={() => checkRenderStatus(latestRender.render_id)} disabled={isRefreshing}>
