...
export const FacebookPreviewContent = ({
  ...
  const saveAsTemplate = async () => {
    ...
    try {
      ...
      const {
        error: insertError
      } = await supabase.from('facebook_templates').insert({
        user_id: user.id,
        name: templateName,
  ...
  const renderMediaContent = () => {
    if (showSlideshow && slideshowUrl) {
      return <div className="aspect-video bg-black rounded-md overflow-hidden">
          <video src={slideshowUrl} className="w-full h-full object-cover" controls autoPlay muted loop />
        </div>;
    } else if (selectedImages.length > 0) {
      return <div className="relative">
          <div className="aspect-square bg-muted rounded-md overflow-hidden mb-3">
            <img src={selectedImages[0]} alt="Preview" className="w-full h-full object-contain" />
          </div>
          {selectedImages.length > 1 && <div className="absolute top-2 right-2 bg-black/70 text-white text-xs px-2 py-1 rounded">
  ...
  return <div className="glass border border-border/40 rounded-lg p-4 w-full max-w-full">
      ...
      {isLoading ? <div className="flex items-center justify-center py-8">
          ...
          <div className="flex flex-col space-y-4">
            ...
            <div className="flex flex-wrap justify-end gap-2 mb-2">
              ...
              <Button variant="outline" size="sm" onClick={() => setIsTemplateDialogOpen(true)} disabled={!generatedText}>
                <Save className="w-4 h-4 mr-2" />
                Sauvegarder comme template
              </Button>
            </div>
            
            <Textarea value={generatedText} onChange={e => onTextChange(e.target.value)} className="mb-4 min-h-[150px]" placeholder="Entrez votre texte ici..." />
            ...
            {!showSlideshow && images && images.length > 0 && <>
                ...
                <ScrollArea className="h-[400px] pr-4">
                  <div className="grid grid-cols-2 gap-2">
                    {images.map((image, index) => <div key={index} className="relative group border-2 border-transparent hover:border-primary rounded-lg transition-all duration-200">
                        <img src={image} alt={`Image ${index + 1}`} className="w-full aspect-square object-cover rounded" />
                        <div className="absolute top-2 left-2 bg-black/50 p-1.5 rounded">
                          <Checkbox checked={selectedImages.includes(image)} onCheckedChange={() => handleImageSelect(image)} className="data-[state=checked]:bg-primary data-[state=checked]:border-primary" />
                        </div>
            ...
            {!showSlideshow && (!images || images.length === 0) && <div className="flex flex-col items-center justify-center h-[200px] bg-muted/20 rounded p-8 text-center">
      ...
      <Dialog open={isTemplateDialogOpen} onOpenChange={setIsTemplateDialogOpen}>
        <DialogContent>
          ...
          <div className="space-y-4 py-4">
            <Input placeholder="Nom du template" value={templateName} onChange={e => setTemplateName(e.target.value)} />
          </div>
          <DialogFooter>
